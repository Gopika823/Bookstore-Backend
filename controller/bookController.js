...
exports.getAllBookController = async (req, res) => {
  ...
  try {
    const query = {
      ...
      // Optional: exclude books by this user if you want
      // userMail: { $ne: email },
      status: { $ne: "pending" } // show only approved books
    };

    const allBooks = await books.find(query).sort({ _id: -1 });
  ...
  const email = req.payload
    try{
        ...
        const session = await stripe.checkout.sessions.create({
            ...
            success_url:"https://bookstore-frontend-ten-flax.vercel.app/payment-success",
             // if payment error -url to be shown
             cancel_url:"https://bookstore-frontend-ten-flax.vercel.app/payment-error"
        });
        console.log(session);
